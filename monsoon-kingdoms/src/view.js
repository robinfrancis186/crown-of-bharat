...
export class KingdomView {
  constructor(canvas, onTap) {
    ...
    this.quality='balanced';this.animations={};this.modelTops={};this.mixers=new Map();this.spellAreas=new Map();this.textures={};
  ...
  async load(progress,buildings=[]){
    ...
    for(const[folder,name]of paths)this.detailModel(this.models[name],name,folder);
  ...
  clone(name,parent=this.scene,x=0,z=0,scale=1){const obj=this.models[name].clone(true);obj.position.set(x,0,z);obj.scale.setScalar(scale);parent.add(obj);if(this.animations[name]?.length){const mixer=new THREE.AnimationMixer(obj);mixer.clipAction(this.animations[name][0]).play();mixer.setTime(rand()*2);this.mixers.set(obj,mixer);}return obj;}
  releaseMixer(obj){const mixer=this.mixers.get(obj);if(mixer){mixer.stopAllAction();mixer.uncacheRoot(obj);this.mixers.delete(obj);}}
  makeLandscape(){
    ...
    this.ambientPeople=[];for(let i=0;i<8;i++){const obj=this.clone(i%3===0?'engineer':'guard',this.scene,0,0,.85),phase=i*.78;obj.position.set(Math.sin(phase)*3.5-1.5,0,Math.cos(phase)*16);obj.rotation.y=Math.atan2(Math.cos(phase)*3.5,-Math.sin(phase)*16);this.ambientPeople.push({obj,phase});}
  ...
  setGhost(placing,valid=true){
    ...
    for(const point of cells.slice(0,24)){
      ...
      const model=this.models[placing.type].clone(true);
  ...
  collectionFeedback(building,amounts){
    ...
    for(const [resource,amount]of Object.entries(amounts||{})){
      ...
      for(let i=0;i<count;i++){
        const angle=i/count*Math.PI*2,mesh=this.overlay(new THREE.OctahedronGeometry(.17,0),colors[resource],cell(building.x+building.w/2)+Math.cos(angle)*.6,top+.4,cell(building.z+building.h/2)+Math.sin(angle)*.6,{opacity:1});
  ...
  updateBattle(battle,dt){
    ...
    for(const unit of battle.units){
      ...
      obj.visible=true;obj.position.set(cell(unit.x),flying?2.2+(this.reducedMotion?0:Math.sin(this.time*3+unit.id.length)*.12):!this.reducedMotion&&unit.action==='walk'?Math.abs(Math.sin(this.time*12+unit.id.length))*.1:0,cell(unit.z));const mixer=this.mixers.get(obj);if(mixer)mixer.timeScale=unit.action==='attack'?1.65:unit.action==='walk'?1.2:.7;obj.rotation.y=unit.facing;obj.rotation.z=!this.reducedMotion&&unit.action==='attack'?Math.sin(this.time*15)*.1:0;
      ...
      if(unit.heroId&&unit.rushUntil>battle.elapsed){obj.scale.setScalar(1.25+(this.reducedMotion?0:Math.sin(this.time*9)*.035));}
    }
  ...
  render(dt){this.time+=dt;for(const[obj,mixer]of this.mixers)if(obj.visible&&(!this.reducedMotion||this.mode==='battle'))mixer.update(dt);if(this.ripples)this.ripples.material.opacity=this.reducedMotion?.18:.15+Math.sin(this.time*.7)*.06;if(this.mode==='home'&&!this.reducedMotion)for(const p of this.ambientPeople){const t=this.time*.09+p.phase;p.obj.position.set(Math.sin(t)*3.5-1.5,Math.abs(Math.sin(this.time*8+p.phase))*.06,Math.cos(t)*16);p.obj.rotation.y=Math.atan2(Math.cos(t)*3.5,-Math.sin(t)*16);}
    this.stepEffects(dt);this.stepImpacts(dt);
    for(const[id,el]of this.labels){if(el.hidden)continue;const obj=this.buildings.get(id);if(!obj)continue;const b=obj.userData.building,v=new THREE.Vector3(obj.position.x,(this.modelTops[obj.userData.asset]||3)+.35,obj.position.z).project(this.camera);el.style.transform=`translate(${(v.x*.5+.5)*innerWidth}px,${(-v.y*.5+.5)*innerHeight}px) translate(-50%,-100%)`;}
    if(this.shake>0.001||this.shaking){
      this.shake=Math.max(0,this.shake-dt*2.6);
      this.updateCamera();
      if(this.shake>0.001){
        const power=this.shake*this.shake*1.7;
        this.camera.position.x+=Math.sin(this.time*61)*power;
        this.camera.position.y+=Math.sin(this.time*47+1.7)*power*.7;
        this.camera.position.z+=Math.cos(this.time*53+.6)*power;
        this.camera.updateMatrixWorld();
        this.shaking=true;
      }else this.shaking=false;
    }
  ...
  stepEffects(dt){
    for(let i=this.effects.length-1;i>=0;i--){
      ...
      if(f.kind==='shot'){
        ...
        if(!f.stationary&&!this.reducedMotion){f.mesh.position.y=f.start.y+t*2.4;f.mesh.position.x=f.start.x+Math.cos(f.angle)*t*.7;f.mesh.position.z=f.start.z+Math.sin(f.angle)*t*.7;f.mesh.rotation.y=t*5;}
