...
export const TUTORIAL = [
  { id: 'welcome', title: 'Welcome to Surajgarh', body: 'Drag the ground to look around, pinch or scroll to zoom, and tap any building to inspect it.', cta: 'Look around', action: null, acknowledge: true },
  { id: 'collect', title: 'Collect what your kingdom made', body: 'Rice fields, teak yards and the bazaar fill up on their own, even while you are away. Collect all gathers every ready building at once.', cta: 'Collect all', action: { type: 'collectAll' }, acknowledge: true },
  { id: 'build', title: 'Build a second rice field', body: 'More fields mean more grain, and grain feeds a bigger army. Choose Rice Fields in Build, place it on open ground and confirm.', cta: 'Open Build', action: { type: 'selectBuild', value: 'farm' }, done: s => s.buildings.filter(b => b.type === 'farm').length >= 2 },
  { id: 'capital', title: 'Raise the Taj Mahal', body: 'Your capital sets the level of everything else. Start its next upgrade to open stronger buildings.', cta: 'Review the upgrade', action: { type: 'openUpgrade', value: s => s.buildings.find(b => b.type === 'fort')?.id }, done: s => levelOf(s, 'fort') >= 2 || s.buildings.some(b => b.type === 'fort' && b.readyAt) },
  { id: 'army', title: 'Fill your army camp', body: 'Troops are free and instant here — the only limit is camp space. Quick fill prepares a balanced force in one tap.', cta: 'Open Army', action: { type: 'openPanel', value: 'army' }, done: s => { const c = capacity(s); return c.used >= c.army; } },
  { id: 'attack', title: 'Take the first road', body: 'Scout The River Toll, check its defenses, then deploy along the gold outer band. Unused troops always come home.', cta: 'Scout the road', action: { type: 'startRaid', value: () => RAIDS[0].id }, done: s => s.totalRaids > 0 },
  { id: 'hero', title: 'Send a champion', body: 'Veer leads your attacks and returns after every battle. Deploy him from the outer band, then tap his card again for Battle Cry.', cta: 'Open the Hall of Heroes', action: { type: 'openPanel', value: 'heroes' }, acknowledge: true },
  { id: 'defend', title: 'Defend what you built', body: 'A second watchtower protects your stores while you are away, and Test defenses lets you attack your own layout safely.', cta: 'Open Build', action: { type: 'selectBuild', value: 'archer_tower' }, done: s => s.buildings.filter(b => b.type === 'archer_tower').length >= 2 },
];
